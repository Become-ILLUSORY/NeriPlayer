...
private const val BOOTSTRAP_YOUTUBE_REVERSE_PROXY_ENABLED_KEY = "youtube_reverse_proxy_enabled"
private const val BOOTSTRAP_YOUTUBE_REVERSE_PROXY_BASE_URL_KEY = "youtube_reverse_proxy_base_url"
...
data class BootstrapSettingsSnapshot(
    ...
    val youtubeReverseProxyEnabled: Boolean = false,
    val youtubeReverseProxyBaseUrl: String = "",
    ...
    fun sanitized(): BootstrapSettingsSnapshot {
        return copy(
            youtubeReverseProxyBaseUrl =
                normalizeYouTubeReverseProxyBaseUrl(youtubeReverseProxyBaseUrl).orEmpty(),
    ...
    context.getSharedPreferences(BOOTSTRAP_SNAPSHOT_PREFS, Context.MODE_PRIVATE)
        .edit {
            putBoolean(BOOTSTRAP_SNAPSHOT_READY_KEY, true)
                ...
                .putBoolean(
                    BOOTSTRAP_YOUTUBE_REVERSE_PROXY_ENABLED_KEY,
                    normalizedSnapshot.youtubeReverseProxyEnabled
                )
                .putString(
                    BOOTSTRAP_YOUTUBE_REVERSE_PROXY_BASE_URL_KEY,
                    normalizedSnapshot.youtubeReverseProxyBaseUrl
                )
                .putString(
...
internal fun Preferences.toBootstrapSettingsSnapshot(): BootstrapSettingsSnapshot {
    return BootstrapSettingsSnapshot(
        ...
        youtubeReverseProxyEnabled = this[SettingsKeys.YOUTUBE_REVERSE_PROXY_ENABLED] ?: false,
        youtubeReverseProxyBaseUrl =
            this[SettingsKeys.YOUTUBE_REVERSE_PROXY_BASE_URL].orEmpty(),
...
private fun readCachedBootstrapSettingsSnapshot(context: Context): BootstrapSettingsSnapshot? {
    ...
    return BootstrapSettingsSnapshot(
        ...
        youtubeReverseProxyEnabled = prefs.getBoolean(
            BOOTSTRAP_YOUTUBE_REVERSE_PROXY_ENABLED_KEY,
            false
        ),
        youtubeReverseProxyBaseUrl = prefs.getString(
            BOOTSTRAP_YOUTUBE_REVERSE_PROXY_BASE_URL_KEY,
            null
        ).orEmpty(),
